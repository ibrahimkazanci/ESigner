...
namespace utils
{
    public class SmartCardManagerKimlikNodanSec
    {
        ...
        {
            mSCManager = null; // yeni ayni anda birden fazla kaydin imzalanacagi egitim imzalamalari icin null yaptim. deniyorum.
            if (mSCManager == null)
            {
        ...
        public static SmartCardManagerKimlikNodanSec getInstanceSabitProgramSCM(int desktop)
        {
            if (Program.mSCManager == null)
            {
                Program.mSCManager = new SmartCardManagerKimlikNodanSec(desktop);
                return Program.mSCManager;
            }
            else
            {
                //Check is there any change
                try
                {
                    //If there is a new card in the system, user will select a smartcard. 
                    //Create new SmartCard.
                    if (Program.mSCManager.getSlotCount() < SmartOp.getCardTerminals().Length)
                    {
                        LOGGER.Debug("New card pluged in to system");
                    ...
                    try
                    {
                        availableSerial = StringUtil.ToString(Program.mSCManager.getBasicSmartCard().getSerial());
                    }
                    catch (SmartCardException ex)
                    {
                        LOGGER.Debug("Card removed");
                    ...
                    if (!Program.mSCManager.getSelectedSerialNumber().Equals(availableSerial))
                    {
                        LOGGER.Debug("Serial number changed. New card is placed to system");
                        Program.mSCManager = null;
                        return getInstance(desktop);
        ...
        public SmartCardManagerKimlikNodanSec(int desktop)
        {
            try
            {
                ...
                if (desktop == 0)
                {
                    ...
                    //*******************************
                    // dbden oku
                    MesajiIsle("ParamSQLServer:" + Program.ParamSQLServer + " ParamSlotID:" + Program.ParamSlotID, 0);
                    String SqlCumlesi = "";
                    ...
                    // && Program.ParamAdiSoyadi == Temp.Substring(startIndex2 + 3, endIndex - (startIndex2 + 3)))
                    if (Program.ParamTCKimlikNo == Temp.Substring(startIndex1 + 13, 11))
                    {
